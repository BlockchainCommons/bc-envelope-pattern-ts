/**
 * @blockchaincommons/envelope-pattern - Parser round-trip + AST-shape tests (EP1)
 *
 ...
 * never the `format(tryParseEnvelopePattern(s)) === s` round-trip. Most parser
 ...
 *   2. `display(tryParseEnvelopePattern(s).value) === s` round-trip for every
...
import { tryParseEnvelopePattern, display, type Pattern } from "../src";

/**
 * Parse `src` and assert successful parse + byte-identical round-trip.
 *
 * Equivalent to Rust's `Pattern::tryParseEnvelopePattern(src).unwrap()` followed by
 * `assert_actual_expected!(p.to_string(), src)`.
 */
function parseAndRoundTrip(src: string): Pattern {
  const r = tryParseEnvelopePattern(src);
  if (!r.ok) {
    throw new Error(`tryParseEnvelopePattern(${JSON.stringify(src)}) failed: ${String(r.error)}`);
  }
  expect(r.ok).toBe(true);
  expect(display(r.value)).toBe(src);
  return r.value;
}
