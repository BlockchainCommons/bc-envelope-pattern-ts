/**
 * @blockchaincommons/envelope-pattern — `cbor` pattern path formatting
 *
...
describe("cbor pattern formatted paths (test_cbor_paths_formatted.rs)", () => {
  it("simple array paths", () => {
    ...
    const r = tryParseEnvelopePattern("cbor(/search(number)/)");
  ...
  it("single value `cbor(/number/)` returns just the root envelope", () => {
    ...
    const r = tryParseEnvelopePattern("cbor(/number/)");
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    const paths = patternPaths(r.value, envelope);
  ...
  it("text-search paths — finds 6 text values incl. map keys", () => {
    ...
    const r = tryParseEnvelopePattern("cbor(/search(text)/)");
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    const paths = patternPaths(r.value, envelope);
  ...
  it("no-match patterns return empty paths", () => {
    const envelope = Envelope.from("just text");
    const r = tryParseEnvelopePattern("cbor(/number/)");
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    const paths = patternPaths(r.value, envelope);
