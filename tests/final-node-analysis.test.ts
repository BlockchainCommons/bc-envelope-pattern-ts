/**
 * @blockchaincommons/envelope-pattern — NODE pattern behaviour
 *
...
import { tryParseEnvelopePattern, matches as patternMatches } from "../src";
...
describe("NODE pattern behaviour (test_final_node_analysis.rs)", () => {
  it("matches the Rust assertion-count matrix", () => {
    ...
    const cases: { label: string; envelope: Envelope; expectedAssertions: number }[] = [
      { label: "Plain CBOR", envelope: Envelope.from(42), expectedAssertions: 0 },
      {
        label: "Known value",
        envelope: Envelope.from(new KnownValue(42)),
        expectedAssertions: 0,
      },
      {
        label: "1 assertion",
        envelope: Envelope.from("subject").addAssertion("key", "value"),
