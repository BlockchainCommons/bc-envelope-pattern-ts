/**
 * @blockchaincommons/envelope-pattern — `cbor` pattern dcbor captures
 *
...
import { capture, tryParseEnvelopePattern, pathsWithCaptures } from "../src";
import { formatPaths } from "../src/format";

describe("cbor captures (test_cbor_captures.rs)", () => {
  it("simple dcbor capture: cbor(/@num(42)/) on Envelope.from(42)", () => {
    const envelope = Envelope.from(42);
    const r = tryParseEnvelopePattern("cbor(/@num(42)/)");
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    const { paths: paths, captures: captures } = pathsWithCaptures(r.value, envelope);
    ...
    expect(formatPaths(paths, { captures: captures })).toBe(expected);
  });

  it("capture with search: cbor(/@values(search(number))/) on [1,2,3]", () => {
    const envelope = Envelope.from([1, 2, 3] as unknown as number);
    const r = tryParseEnvelopePattern("cbor(/@values(search(number))/)");
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    const { paths: paths, captures: captures } = pathsWithCaptures(r.value, envelope);
  ...
  it("multiple search captures over a flat string array", () => {
    const envelope = Envelope.from(["name", "Alice", "age", "30"] as unknown as number);
    const r = tryParseEnvelopePattern("cbor(/@names(search(text))/)");
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    const { paths: paths, captures: captures } = pathsWithCaptures(r.value, envelope);
  ...
  it("nested captures: @users(search([@name(text), @score(text)]))", () => {
    ...
    const r = tryParseEnvelopePattern("cbor(/@users(search([@name(text), @score(text)]))/)");
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    const { paths: paths, captures: captures } = pathsWithCaptures(r.value, envelope);
  ...
  it("mixed envelope-level + dcbor-level captures", () => {
    const envelope = Envelope.from(42);
    const cborR = tryParseEnvelopePattern("cbor(/@dcbor_level(42)/)");
    expect(cborR.ok).toBe(true);
    if (!cborR.ok) return;
    const pattern = capture("envelope_level", cborR.value);

    const { paths: paths, captures: captures } = pathsWithCaptures(pattern, envelope);
    expect(paths.length).toBe(1);
  ...
  it("capture-name conflicts merge across levels", () => {
    const envelope = Envelope.from(42);
    const cborR = tryParseEnvelopePattern("cbor(/@same_name(42)/)");
    expect(cborR.ok).toBe(true);
    if (!cborR.ok) return;
  ...
  it('array traversal captures via search(text) on ["hello","42","world","123"]', () => {
    const envelope = Envelope.from(["hello", "42", "world", "123"] as unknown as number);
    const r = tryParseEnvelopePattern("cbor(/@text(search(text))/)");
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    const { paths: paths, captures: captures } = pathsWithCaptures(r.value, envelope);
  ...
  it("no-match: cbor(/@num(number)/) on text envelope returns empty", () => {
    ...
    const r = tryParseEnvelopePattern("cbor(/@num(number)/)");
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    const { paths: paths, captures: captures } = pathsWithCaptures(r.value, envelope);
    ...
    expect(formatPaths(paths, { captures: captures })).toBe("");
  });

  it("performance smoke: cbor(/@nums(search(number))/) on [1,2,3]", () => {
    const envelope = Envelope.from([1, 2, 3] as unknown as number);
    const r = tryParseEnvelopePattern("cbor(/@nums(search(number))/)");
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    const { paths: paths, captures: captures } = pathsWithCaptures(r.value, envelope);
  ...
  it("comprehensive: @data(cbor(/@people(search(text))/)) on [Alice, Bob, Charlie]", () => {
    const envelope = Envelope.from(["Alice", "Bob", "Charlie"] as unknown as number);
    const cborR = tryParseEnvelopePattern("cbor(/@people(search(text))/)");
    expect(cborR.ok).toBe(true);
    if (!cborR.ok) return;
