/**
 * @blockchaincommons/envelope-pattern — Credential pattern tests
 *
...
import { IS_A, ISSUER, CONTROLLER, NOTE } from "@blockchaincommons/known-values";
import type { Digest } from "@blockchaincommons/components";
import { tryParseEnvelopePattern, matches, paths as patternPaths, pathsWithCaptures } from "../src";
...
function alicePrivateKey(): PrivateKeyBase {
  return PrivateKeyBase.from(hexToBytes(ALICE_SEED_HEX));
}
...
function credential(): Envelope {
  const arid = ARID.from(hexToBytes(CREDENTIAL_ARID_HEX));
  ...
  const signed = wrapped.addSignature(schnorrSigner, { signing: { type: "Schnorr", rng } });
...
function redactedCredential(env: Envelope): Envelope {
  ...
  const content = env.subject().unwrap();
  ...
  return env.elide({ revealing: target });
}
...
describe("Credential pattern tests (credential_tests.rs, real crypto)", () => {
  it("credential() produces an envelope with 13 inner assertions + 2 outer", () => {
    ...
    const inner = env.subject().unwrap();
  ...
  it("`search(assertobj(text|number))` finds 11 paths in the credential", () => {
    ...
    const env = credential();
    const r = tryParseEnvelopePattern("search(assertobj(text|number))");
  ...
  it('`search(assertpred("firstName")->obj("James"))` finds exactly 1 path', () => {
    ...
    const env = credential();
    const r = tryParseEnvelopePattern('search(assertpred("firstName")->obj("James"))');
    expect(r.ok).toBe(true);
    if (!r.ok) return;
  ...
  it('`search(@cap(assertpred("firstName")->obj("James")))` propagates the capture', () => {
    ...
    const env = credential();
    const r = tryParseEnvelopePattern('search(@cap(assertpred("firstName")->obj("James")))');
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    const { paths: paths, captures: captures } = pathsWithCaptures(r.value, env);
  ...
  it("real elision: redactedCredential matches `search(elided)` non-trivially", () => {
    ...
    const r = tryParseEnvelopePattern("search(elided)");
    expect(r.ok).toBe(true);
    if (!r.ok) return;
  ...
  it("redactedCredential elides the photo assertion (non-revealed)", () => {
    ...
    const photoR = tryParseEnvelopePattern('search(assertpred("photo"))');
  ...
  it("`search(node({13}))` finds the inner node (13 assertions)", () => {
    const env = credential();
    const r = tryParseEnvelopePattern("search(node({13}))");
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    expect(patternPaths(r.value, env).length).toBe(1);
  });
  ...
  it("non-existent predicate matches nothing (sanity)", () => {
    const env = credential();
    const r = tryParseEnvelopePattern('search(assertpred("nonExistent"))');
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    expect(matches(r.value, env)).toBe(false);
