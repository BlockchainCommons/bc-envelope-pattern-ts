/**
 * @blockchaincommons/envelope-pattern — `leaf` keyword parsing
 *
...
import { tryParseEnvelopePattern, display } from "../src";
...
describe("Leaf keyword parsing (test_leaf_parsing.rs)", () => {
  it("parses `leaf` as a Structure pattern", () => {
    ...
    if (r.ok) {
      ...
      expect(r.value.kind).toBe("Structure");
    }
  });
  ...
  it("displays `leaf` as `leaf`", () => {
    ...
    expect(r.ok).toBe(true);
    if (r.ok) {
      expect(display(r.value)).toBe("leaf");
    }
  });
});
